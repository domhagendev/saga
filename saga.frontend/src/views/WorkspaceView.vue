...
// Sidebar page list — always includes a virtual placeholder for the page
// currently open so it shows up in the nav even before generation.
const sidebarPages = computed(() => {
  const real = bookStore.sortedPages
  const hasCurrentPage = real.some((p) => p.pageNr === currentPageNr.value)
  if (hasCurrentPage) return real
  const placeholder = {
    bookId: bookStore.currentBook?.bookId ?? '',
    pageNr: currentPageNr.value,
    content: '',
    userNote: '',
    targetMood: '',
    orderIndex: currentPageNr.value,
  }
  return [...real, placeholder].sort((a, b) => a.pageNr - b.pageNr)
})
...
async function handleEdit(beat: string, mood: string, mentionedIds: string[]): Promise<void> {
  isGenerating.value = true
  generateError.value = null
  try {
    const updated = await bookStore.editPage(currentPageNr.value, beat, mood, mentionedIds)
    if (!updated) {
      generateError.value = t('workspace.generateFailed')
    }
  } catch (error) {
    ...
    <div v-else class="flex flex-1 overflow-hidden bg-stone-200">
      ...
      <div class="flex flex-1 flex-col overflow-hidden pb-12 bg-stone-200 dark:bg-stone-900/40">
        <div class="flex flex-1 flex-col overflow-hidden">
          <div ref="storyContentRef" class="flex min-h-0 flex-1 flex-col overflow-hidden pb-0">
            <div class="mx-auto flex w-full max-w-3xl min-h-0 flex-1 flex-col">
              <StoryPageComponent
                :page="currentPage ?? { pageNr: currentPageNr, bookId: bookStore.currentBook?.bookId ?? '', content: '', userNote: '', targetMood: '', orderIndex: currentPageNr }"
        ...
        <div class="shrink-0 bg-stone-200 dark:border-stone-800 dark:bg-stone-950">
      ...
      <aside class="my-4 w-80 shrink-0 border-l border-stone-200 bg-stone-200 dark:border-stone-800 dark:bg-stone-950/80">
        <Tabs default-value="atlas" class="flex h-full flex-col">
          ...
          <TabsContent value="summary" class="mt-0 flex-1 overflow-hidden">
            <ScrollArea class="h-full">
              <div class="px-2 py-3">
                <div class="rounded-xl bg-stone-50 dark:bg-stone-900/60">
                  <div class="px-4 py-3">
